...
var listMap = {
    ...
    proxy: function(name,cb) { 
        listResource("^(\\/integrationPackage\\/).+",cb);
    },
    resource: function(name,cb) {
        listResource("^(?!\\/tasklets\\/).+",cb);
    },
    sample: function(name,cb) {
        listResource("^(\\/samples\\/).+",cb);
    },
...
function listResource(regex,cb) {
    var query = {
        "op": "SELECT",
        "targetType": "CdmResource",
        "properties": attributes,
        "where": {"name": {"$regex": regex}},
        "options": {"limit": 1000, "sort":"name"}
    };
    if(!common.global.argv.includeGlobal) {
        query.where.createdBy = {"$ne":"mms"};
    }
