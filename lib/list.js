...
var listMap = {
    app: function(name,cb) {
        ...
        if(name) {
            ...
            common.global.session.directive({
                ...
                "where": {"taskletId": {"$regex": nameExpr}},
                "options": {"limit": 1000, "sort":{"taskletId":1}}
            },function(err,res) {
    ...
    model: function(name,cb) {
        ...
        // don't get porperties and operations
        var nameExpr = convertWildcardsToRegex(name);
        if(nameExpr && nameExpr.indexOf(".*") != -1) {
            name = null;
        }
    ...
    proxy: function(name,cb) {
        listResource("^(\\/integrationPackage\\/)",name,cb);
    },
    resource: function(name,cb) {
        listResource("^(?!\\/tasklets\\/)",name,cb);
    },
    sample: function(name,cb) {
        listResource("^(\\/samples\\/)",name,cb);
    },
...
function listResource(prefix,name,cb) {
    var regex = prefix;
    var nameExpr = convertWildcardsToRegex(name);
    if(nameExpr) {
        regex = regex + nameExpr;
    } else {
        regex = regex + ".+";
    }
