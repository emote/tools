...
exports.showLog = function showLog(theSession,theArgV,startTime,endTime)
{
    argv = theArgV;
    session = theSession;
    ...
    //
    //  If this is a regular and they didn't override loggerName or logLevel
    //  set the defaults that show only proxy output, which is the standard use case.
    //
    if (theSession.username != "mms")
    {
        if (!argv.loggerName)
        {
            argv.loggerName = "Node.js";
        }
        if (!argv.logLevel)
        {
            argv.logLevel = "INFO";
    ...
    if (argv.loggerName == "ALL")
    {
        argv.loggerName = null;
    }
    if (argv.logLevel == "ALL")
    {
        argv.logLevel = null;
    }
...
function convertTimeStringToDate(s)
{
        ...
        ||  (lastChar == "h"))
    {
        ...
        {
            //console.log("Assuming server clock is set to local time");
        ...
        if (resultObject.status == 'SUCCESS')
        {
            ...
            for (var i=0; i<resultObject.results.length; i++)
            {
                ...
                else
                {
                    ...
                    //
                    if (session.username == 'mms') out.push(tenant);
