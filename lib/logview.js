...
exports.showLog = function showLog(startTime,endTime)
{
    ...
    obj.properties = ["time","message","tenant","level","logger","username","stackTrace","taskletId"];
    obj.options = new Object();
    obj.options.limit = ROW_LIMIT;
    obj.options.startAt = 0;
    obj.options.sort = {time:1};
...
function convertTimeStringToDate(s)
{
        ...
        ||  (lastChar == "h"))
    {
        ...
        if (    (session.baseURLString.indexOf("localhost") >= 0)
            ||  (session.baseURLString.indexOf("10.220") >= 0)
            ||  (session.baseURLString.indexOf("192.") >= 0))
        {
            console.log("Assuming server clock is set to local time for localhost");
...
function onGetLogRecords(err,resultObject)
{
    ...
    if (resultObject && (resultObject.targetType == 'RestResponse'))
    {
        if (resultObject.status == 'SUCCESS')
        {
            ...
            for (var i=0; i<resultObject.results.length; i++)
            {
                ...
                if (!argv.showNoise)
                {
                    ...
                    {
                        //
                        //  Hide the "get log" messages this command generates
                        //
                        ...
                        //
                        //  Hide the "get minimum version" messages this command generates
                        //
                        else if (logEntry.message.indexOf('REST REQUEST: {"op":"INVOKE","targetType":"CdmSystemStatus","name":"systemProperty","params":{"name":"emoteMinimumVersion"}') == 0)
                        {
                            skip = true;
                        }
                        else
                    ...
                    if (!skip && ((logEntry.taskletId == "system.ping") || (logEntry.taskletId == "system.readLog")))
                    {
                        skip = true;
                    }
                ...
                //
                //  Otherwise just format it for human consumption
                //
                else
                {
