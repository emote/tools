...
function uploadResourceToMMS(proxyName,data,callback) {
    ...
    var parts = {
        resourceName: getProxyPrefix() + proxyName,
...
exports.deployers = {
    proxy : function(dirname,cb) {
        ...
        var packageJSON = common.getJSON(path.join(dirname,"package.json"));
        ...
        if(!pkgVersion) common.exit('version missing from package.json in proxy ' + pkg);

        common.pushdProjectHome(function(callback) {

            try {
                process.chdir(path.join('staging','proxy',dirname,'proxyContents'));
            } catch (err) {
                common.exit('The proxy ' + dirname + ' must be built before it can be deployed.\nUse "emote build"')
            }

            var data = zipper.zipDirToBuffer(".",ignoreFiles);
            ...
            session.directive({
                    ...
                    where: {name : getProxyPrefix() + dirname},
                    properties: ["id"]
                },
                null, function(err, result) {
                    ...
                    var rsrcId = result.results[0].id;
                    session.directive({
                            op: "UPDATE",
                            targetType: "CdmResource",
                            where: {id : rsrcId},
                            values: {description : pkgName + " proxy, version " + pkgVersion}
                        },
                        null, function(err, result) {
                            if (err) {
                                return cb(err);
            ...
            async.forEachSeries(resourceRequest,function(parms,cb2)
                {
                    ...
                    switch (suffix)
                    {
                        case "gif":
                            ...
                            break;
                        case "pdf":
                            parms.contentType = "application/pdf";
                            break;
                        case "png":
...
function outputWriter(err,result,context,cb) {
    if(err) {
        console.log("Error:");
        console.dir(err);
    }
