...
var deployers = {
    ...
    theme : function(dirname,cb) {
        ...
        theme.update(session,dirname,cb,false,exports.global.argv.markThemePublic);
    }

};

function doForAllDirs(f,cb) {
...
function deployAll(dirname,cb) {
    ...
    if(fs.existsSync(dirname)) {
        ...
        doForAllDirs(deployers[dirname],function(err) {
            process.chdir('..');
...
exports.command = function deploy(type, name, mmsSession, cb) {
    ...
    if(!type && !name) {
        ...
        async.forEachSeries(['model','proxy','app','operation'],deployAll,function(err) {
            if(err) {
