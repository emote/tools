...
function doBrandingPhase()
{
    ...
    switch (themePhase)
    {
        case 1:
            ...
            //
            if (!deleteAll && mmcWhitelabelDirectoryPath)
            {
        ...
        case 2:
            ...
            //  The caller didn't ask for "--delete" *AND* the directory exists so we will be updating the branding elements
            //
            if (!deleteAll && mmcBrandingDirectoryPath)
            {
...
function updateMMCBrandingElement()
{
    ...
    var suffix = brandingElementResourceName.substring(brandingElementResourceName.lastIndexOf(".")+1);
    ...
    if (!brandingContentType)
    {
        console.log("Resource " + brandingElementResourceName + " has an unknown suffix");
        process.exit(1);
    }
    ...
    fs.readFile(srcFile, null, function (err, data)
    {
        if (err)
        {
            throw err;
