...
function addCommands(commandMap) {
    ...
    commandMap.generateFromWsdl = function() {
        var command = emote.getCommand();
        generate(command[1], emote.finished)
    };

    commandMap.getWsdl = function() {
        var command = emote.getCommand();
        emote.mmsLogin(function() {
            getWsdl(common.global.session, command[1], command[2], command[3], command[4], command[5], command[6], emote.finished);
        })
    };
}
...
function generate(subProject, cb) {
    ...
    createModel(subProject, function(err, model) {
        ...
        createProxy(subProject, model, function(err) {
            ...
            create.add("proxy", subProject, "_wsdl", cb);
        });
    });
}
