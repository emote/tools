...
exports.getResourceAndWriteFiles = download;
...
function listSamples(callback) {
    var filter = '/samples/.*';

    resourceQuery(filter,function(err,result) {
        if(err) {
            common.exit('Error selecting resources ' + filter + " : " + err);
        }

        var max_width = 0;
        result.results.forEach(function(row) {
            row.name = row.name.substring(row.name.lastIndexOf('/')+1,row.name.lastIndexOf('.zip'));
            max_width = Math.max(max_width,row.name.length);
        });        

        result.results.forEach(function(row) {
            console.log(pad(row.name,max_width+2) + row.description);
        });
        callback();
    });
}

function download(artifactType,name,callback) {
