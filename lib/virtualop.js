...
{    
    if (restResponse.status == "SUCCESS")
    {
        console.log('Type "' + opInfo.type + '" deleted');
    }
    else if (	restResponse.errors
			&& 	(restResponse.errors.length == 1)
    ...
    if (opInfo.returns)
    {
        var returnTypesToAdd = opInfo.returns.split(",");
        
        for (var i=0; i<returnTypesToAdd.length; i++)
        {
            returns.push(returnTypesToAdd[i]);
        }  
    }
...
function createOperation(err,result) {
    ...
    {
        console.log('Create type "' + opInfo.type + '" succeeded');
    }
    else
    {
        console.log('Create type "' + opInfo.type + '" failed: ' + processRestResponse(result));
        process.exit(1);
    }
    ...
    console.log("Creating Virtual Type '" + opInfo.type + "' with Javascript operation '" + opInfo.name + "' from file " + filename);
...
function onRegisterComplete(err,result)
{
    if (result.status == "SUCCESS")
    {
        console.log('Create operation "' + opInfo.name + '" succeeded');
    }
    else
    {
