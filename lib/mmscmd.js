...
exports.updateProxyDigest = function(sessionParam,extSysName,proxyDigest,cb) {
    session = sessionParam;
    updateProxyDigest(extSysName,proxyDigest,cb);
} 
...
function cleanTenant(sessionParam) {
    ...
    session.onDirectivesComplete(function(err) {
        console.log('All external systems and bindings removed from tenant.');
    });
}
...
function updateProxyDigest(extSysName,proxyDigest,cb) {
    session.directive({
        op: 'SELECT',
        targetType: 'CdmExternalSystem',
        properties: ['id','name'],
        ...
        if(!result.results || result.results.length != 1) {
            console.log("No unique CdmExternalSystem found with name " + extSysName);
            process.exit(1);
        }
        var extSysId = result.results[0].id;

        session.directive({
            op: 'UPDATE',
            targetType: 'CdmExternalSystem',
            values: {proxyDigest: proxyDigest},
            where: {id: extSysId}
        },cb);
    });
}
