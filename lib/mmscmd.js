...
function execDirective(d,cb) {
    if(d.op == "INVOKE" && d.targetType == "CdmExternalSystem" && d.name == "createExternalSystem") {
    ...
    } else if(false && d.op == "INVOKE" && d.targetType == "CdmExternalSystem" && d.name == "createExternalSystem") {

        session.directive({
            op: 'SELECT',
            targetType: 'CdmExternalSystem',
            properties: ['id','name'],
            where: {name: d.params.name}
        },function(err,result) {
            console.log('SELECT CdmExternalSystem');
            console.dir(err);
            console.dir(result);

            if(err) return cb(err);

            deleteResultList(err,result,function(err) {
                ...
                session.directive({
                    ...
                    console.log('CREATED CdmExternalSystem: ');
                    //console.dir(result);
                    cb(err,result);
                });
            });
    ...
    } else if(false && d.op == "INVOKE" && d.targetType == "CdmType" && d.name == "bindCdmType") {
        ...
        session.directive({
            ...
            var cdmProperties = result.results;
            var propList = [];
            cdmProperties.every(function(row) {
            ...
            session.directive({
                ...
                console.log('Inserted CdmTargetType: ' + externalType);
                ...
                session.directive({
                    ...
                    cdmProperties.every(function(bindProp) {

                        var cdmProp = bindProp.name;
