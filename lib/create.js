...
function addFilesFromDir(templateDir,partPath,targetPath,callback) {
    ...
    async.forEachSeries(templateFiles,
        function(filename,cb) {
            ...
            copyFile(filename, relativeName, cb)
...
function copyFile(src, target, cb) {
    // console.log("About to copy " + src + " to " + target);
    var rd = fs.createReadStream(src);
    rd.on("error", function(ex) {
        console.log("Error reading from file " + src + ": " + ex);
    });
    var wr = fs.createWriteStream(target);
    wr.on("error", function(ex) {
        console.log("Error writing to file " + target + ": " + ex);
    });
    wr.on("close", function(ex) {
        // console.log("Done copying " + src + " to " + target);
        cb();
    });
    rd.pipe(wr);
}
