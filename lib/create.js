    ...
    session.directive({
        ...
        async.forEachSeries(result.results,
            function(row,cb) {
                ...
                var resourcePath = row.name + '?raw=true';
                session.getResource(resourcePath,function(err,body) {
                    var filename = resourceName;
                    if (path.sep != '/') {
                        filename = resourceName.replace(/\//g, path.sep);
                    }
...
function npmInstallForGeneratedPackages(callback) {
    ...
    async.forEachSeries(packageList,installWithNpm,function(err) {
        if(err) common.exit('Failure on npm install of generated packages: ' + err);
...
function installWithNpm(pkg,callback) {
    npm.load({},function(err,npmobj) {
        ...
        var name = common.getJSON(path.join(pkg,"package.json")).name;
        if(!name) common.exit('name missing from package.json in proxy ' + pkg);
        npmobj.install(pkg,function(err) {
            if(err) common.exit("Error in NPM install of " + pkg);
            rimraf(pkg,function(err) {
                if(err) common.exit('Failure "rm -rf" of generated packages: ' + err);
                // Add a small delay before the rename because we detected an
                // intermittant race condition using rimraf on Windows
                setTimeout(function() {
                    var currentName = path.join("node_modules",name);
                    console.log('renaming ' + currentName + ' to ' + pkg);
                    fs.renameSync(currentName,pkg);
                    callback();
                },1000);                          
            });
        });
