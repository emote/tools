...
exports.deployModel = function deployModel(externalSystemName,mmsSession,cb) {
    ...
    if(!credentials.host) {
        console.log("externalSystemName for " + externalSystemName + " must contain the Salesforce host string.");
        process.exit(1);
    }
    ...
    emsfdc.sfdcSess.getSession(credentials,function(err,sfSess) {
        if(err) {
            console.log("Error logging into Salesforce with provided credentials:");
...
function createExternalSystem(err) {
    ...
    session.directive({
        ...
        values: {
            ...
            proxyConfiguration: {verbose: verboseLoggingForExternalSystem}
        }
...
function createMappedCustomObjects() {
    ...
    sfSession.restRequest('GET','sobjects/',null,function scanSObjects(err,result) {
        if(err) {
            console.log("Error getting list of Saleforce objects: " + err);
            process.exit(1);
        }

        var sobjects = result.jsonBody.sobjects;
...
function getBasicInfo(item,cb) {
    sfSession.restRequest('GET',item.basicInfo.urls['describe'],null,function createCdmTargetType(err,result) {
        if(err) {
            console.log("Error getting decription of Saleforce object: " + err);
            process.exit(1);
        }
        processSalesforceType(result.jsonBody,item.foundType,cb);
    });
}
...
function processSalesforceType(result,typeSubset,cb) { 
    ...
    attributes.forEach(function(attr) {
        ...
        || attr.name == "SystemModstamp" || attr.name == "CreatedDate" || attr.name == "CreatedById") {
...
function createBinding(cdmName,sfdcName,propList,typeSubset,callback) { 
    ...
    async.series([
        function(cb) {
            mmscmd.remove(session,'CdmTargetType',{name: sfdcName},cb);
        },
        function(cb) {
            mmscmd.remove(session,'CdmTypeBinding',{name: cdmName},cb);
        },
        function(cb) {
            mmscmd.remove(session,'CdmPropertyBinding',{cdmType: cdmName},cb);
        },
        function(cb) {
            session.directive({
                op: 'INSERT',
    ...
    session.directive({
        ...
        session.directive({
            ...
            params: {
                ...
                indices: [[{property: 'externalId', order: 1}, {property: 'tenantId', order: 2}, {options: {unique: false}}]]
            }
        },callback);
    });
}
...
// Adds or replaces a CDM enumeration based on a table in Saleforce
    ...
    sfSession.restRequest('GET','query/?q=' + encodeURIComponent(query),null,function(err,selectResult) {
        if(err) {
        ...
        var records = selectResult.jsonBody.records;
