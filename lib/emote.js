...
var commandMap = {
    ...
    log: function () {
        mmsLogin(function() {
            logview.tail(session,command[1],argv.refreshInterval,argv.loggerName,argv.logLevel);
        });         
    },
    templates : function() {
    ...
    test: function() { 
        mmsLogin(function() {
            runtest.command(command[1],session,finished); 
        }); 
    },
    undeploy: function() {
        mmsLogin(function() {
...
function main() {
    argv = optimist.usage('Usage: $0')
        ...
        .options('loggerName', {alias:'n', describe: 'Log messages to include when monitoring MMS log', "default": "Node.js"})
        .options('logLevel', {alias:'l', describe: 'Log level to show when monitoring MMS log', "default": "INFO"})
...
function mmsLogin(callback) {
    ...
    if(!argv.allowGlobal && (mmsCreds.username === "mms")) {
...
function startRegressionTest() {
    ...
    var testScript = path.join(__dirname,"..","test","run.sh");
