...
var commandMap = {
    add: function() { 
        create.add(command[2],command[3],null,modelInit,command[1],session,finished);
    },
    create: function() { 
...
function mmsLogin(callback) {
    ...
    if(argv.username || argv.password || argv.server != DEFAULT_SERVER) {
        if(!(argv.username && argv.password)) {
            console.log("username and password must be specified on the command line")
            process.exit(1);
        }
    ...
    } else if(process.env.EMOTE_PROFILE) {
        try {
            mmsCreds = JSON.parse(process.env.EMOTE_PROFILE);
        } catch(err) {
            console.log('Cannot parse JSON in EMOTE_PROFILE: ' + err);
            process.exit(1);
        }
    ...
    session.realmId = mmsCreds.realmId;
    session.token = mmsCreds.token;
