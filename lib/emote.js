...
var commandMap = {
    ...
    list : function() {
        if(command[1] === 'templates') {
            ...
            mmsLogin(function() {
                download.listSamples();
            });
        } else {
            console.log('Must list one of: samples, templates');
        }
    },    
    log: function () {
        mmsLogin(function() {
...
function getCredentials(cb /*function(credentials)*/) {
    ...
    var mmsCreds = {server:DEFAULT_SERVER};

    if(argv.profile) {
        common.verbose("Reading profile specified on command line.");
        mergeCredentials(mmsCreds,readProfile(argv.profile));
    } else {
        if(fs.existsSync(EMOTE_PROFILE)) {
            common.verbose("Reading profile from " + EMOTE_PROFILE);
            mergeCredentials(mmsCreds,readProfile(EMOTE_PROFILE));
        }
        if(fs.existsSync(PROJECT_PROFILE)) {
            common.verbose("Reading profile from project root.");
            mergeCredentials(mmsCreds,readProfile(PROJECT_PROFILE));
        }
    }
